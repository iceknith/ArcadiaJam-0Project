...
### This file has been modified by the creator of this game, ###
### in order to fit the expected usage of the virtual keayboard in the game ###
...
signal key_pressed(unicode_value:int)
...
func _switch_layout(key_data):
	...
	if key_data.get("layout-name") == "PREVIOUS-LAYOUT":
		if prev_prev_layout != null:
			...
			#set the focus to the layout switch key
			var VBox = prev_prev_layout.get_children()[0]
			for HBox in VBox.get_children():
				for button in HBox.get_children():
					if(button.is_connected("released", _switch_layout)):
						button.grab_focus()
						return
			return
	...
	for layout in layouts:
		if layout.get_meta("layout_name") == key_data.get("layout-name"):
			_show_layout(layout)
			#set the focus to the layout switch key
			var VBox = layout.get_children()[0]
			for HBox in VBox.get_children():
				for button in HBox.get_children():
					if(button.is_connected("released", _switch_layout)):
						button.grab_focus()
	...
	for layout in data.get("layouts"):
		...
		for row in layout.get("rows"):
			...
			for key in row.get("keys"):
				...
				_set_key_style("focus",new_key, style_hover)
