...
func _on_world_player_signal(signalType, signalValues):
	if (signalType == "death"):
		...
		for i in signalValues[0].size():
			...
			elif signalValues[1][i] == "passive":
				pass
		$Shop.visible = true
		$Shop.init_item_selection(iconPath, signalValues[0], signalValues[1])
...
func _on_shop_player_gain_spell(spell_num:int, spellPointer:PackedScene):
	GUISignal.emit("spell_change", [spell_num, spellPointer])
