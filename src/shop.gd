...
signal player_gain_passive
signal player_gain_spell(spell_num:int, spellPointer:PackedScene)
signal dialogue_end
...
func init_item_selection(itemIconPath:Array, itemPointer:Array, itemType:Array):
	...
	for i in 3:
		...
		if (i < itemIconPath.size()):
			...
			if (itemTypes[i] == "spell"):
				var spell:Spell = itemPointers[i].instantiate()
				marker.get_node("description").text = str(spell.name, 
				"\n Coût: ", spell.cost, " ", spell.costType, "\n Max Vie: +", spell.maxColorAdd, " ", spell.costType)
		else:
...
func select_item(index:int):
	...
	if (itemTypes[index] == "ability"):
		player_gain_passive.emit()
		dialogue_end.emit()
...
func select_spell(index:int):
	player_gain_spell.emit(index, selectedItem)
	dialogue_end.emit()
